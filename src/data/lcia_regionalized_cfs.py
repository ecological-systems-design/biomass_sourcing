...
from src.other.name_match import (get_country_match_df, get_country_match_df_globiom, get_country_match_df_aware,
...
def calculate_area_per_country_and_land_use():
    ...
    land_use_map = r"data/external/GLAM_land_intensity_5min.tif"
...
def calculate_area_weighted_regional_water_cfs():
    df_cf = pd.read_excel(r'data/external/AWARE_water_CF.xlsx', engine='openpyxl', sheet_name='AWARE-annual')
    ...
    for loc in loc_list:
        if loc not in list(df_cf.Location.unique()):
            ...
            df_temp['Location'] = loc
            df_cf = pd.concat([df_cf, df_temp], ignore_index=True)
    df_cf.to_csv('data/interim/cf_aware_processed.csv')
    return df_cf
