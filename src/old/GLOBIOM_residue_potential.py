...
def forest_cnc_ratio_single(forest_residue):
    df_cnc = read_globiom_forest_cnc_ratio()
    residue_name_c = f'{forest_residue}_C'
    residue_name_nc = f'{forest_residue}_NC'
    df_cnc_single_0 = df_cnc[(df_cnc.PRODUCT == residue_name_c) |
                             (df_cnc.PRODUCT == residue_name_nc)].copy()
    df_cnc_single_0 = df_cnc_single_0.fillna(0)
    df_cnc_single = pd.DataFrame(columns=["REGION", "SCENARIO", "YEAR", "RATIO_C"])
    for sce in list(df_cnc_single_0.SCENARIO.unique()):
        for reg in list(df_cnc_single_0.REGION.unique()):
            df_temp = df_cnc_single_0[(df_cnc_single_0.REGION == reg) & (df_cnc_single_0.SCENARIO == sce)].copy()
            for col in list(df_temp.columns)[5:]:
                yr = int(col[-4:])
                df_temp = df_temp.loc[df_temp[col] != 'Eps'].copy()
                if df_temp.shape[0] == 1:
                    if residue_name_c in list(df_temp.PRODUCT.unique()):
                        ratio_c = 1
                    else:
                        ratio_c = 0
                    df_temp2 = pd.DataFrame(columns=["REGION", "SCENARIO", "YEAR", "RATIO_C"],
                                            data=[[reg, sce, yr, ratio_c]])
                    df_cnc_single = pd.concat([df_cnc_single, df_temp2], ignore_index=True)
                elif df_temp.shape[0] == 2:
                    val_c = df_temp.loc[df_temp.PRODUCT == residue_name_c, col].astype(float).sum()
                    val_nc = df_temp.loc[df_temp.PRODUCT == residue_name_nc, col].astype(float).sum()
                    if val_nc + val_c > 0:
                        ratio_c = val_c / (val_c + val_nc)
                        df_temp2 = pd.DataFrame(columns=["REGION", "SCENARIO", "YEAR", "RATIO_C"],
                                                data=[[reg, sce, yr, ratio_c]])
                        df_cnc_single = pd.concat([df_cnc_single, df_temp2], ignore_index=True)
    for sce in list(df_cnc_single.SCENARIO.unique()):
        for reg in list(df_cnc_single.REGION.unique()):
            df_temp = df_cnc_single[(df_cnc_single.REGION == reg) &
                                    (df_cnc_single.SCENARIO == sce)].copy()
            if df_temp.shape[0] == 0:  # use the other scenario as proxy
                df_temp = df_cnc_single[(df_cnc_single.REGION == reg) &
                                        (df_cnc_single.SCENARIO == "scenRCP1p9")].copy()
                if df_temp.shape[0] == 0:
                    df_temp = df_cnc_single[(df_cnc_single.REGION == reg) &
                                            (df_cnc_single.SCENARIO == "scenRCPref")].copy()
                    year_list = list(df_temp.YEAR.unique())
                    proxy_yr = min(year_list, key=lambda x: abs(x - 2020))
                    for yr in range(2000, 2101, 10):
                        ratio_c = df_cnc_single.loc[(df_cnc_single.REGION == reg) &
                                                    (df_cnc_single.SCENARIO == "scenRCPref") &
                                                    (df_cnc_single.YEAR == proxy_yr), "RATIO_C"].iloc[0]
                        df_temp2 = pd.DataFrame(columns=["REGION", "SCENARIO", "YEAR", "RATIO_C"],
                                                data=[[reg, sce, yr, ratio_c]])
                        df_cnc_single = pd.concat([df_cnc_single, df_temp2], ignore_index=True)
                else:
                    ...
                    for yr in range(2000, 2101, 10):
                        ratio_c = df_cnc_single.loc[(df_cnc_single.REGION == reg) &
                                                    (df_cnc_single.SCENARIO == "scenRCP1p9") &
                                                    (df_cnc_single.YEAR == proxy_yr), "RATIO_C"].iloc[0]
                        df_temp2 = pd.DataFrame(columns=["REGION", "SCENARIO", "YEAR", "RATIO_C"],
                                                data=[[reg, sce, yr, ratio_c]])
                        df_cnc_single = pd.concat([df_cnc_single, df_temp2], ignore_index=True)
            elif df_temp.shape[0] != 11:
                year_list = list(df_temp.YEAR.unique())
                proxy_yr = min(year_list, key=lambda x: abs(x - 2020))
                for yr in range(2000, 2101, 10):
                    if yr not in year_list:
                        ratio_c = df_cnc_single.loc[(df_cnc_single.REGION == reg) &
                                                    (df_cnc_single.SCENARIO == sce) &
                                                    (df_cnc_single.YEAR == proxy_yr), "RATIO_C"].iloc[0]
                        df_temp2 = pd.DataFrame(columns=["REGION", "SCENARIO", "YEAR", "RATIO_C"],
                                                data=[[reg, sce, yr, ratio_c]])
                        df_cnc_single = pd.concat([df_cnc_single, df_temp2], ignore_index=True)
    df_cnc_single['PRODUCT'] = forest_residue

    return df_cnc_single
...
def compile_forest_cnc_ratio():
    df_sw = forest_cnc_ratio_single('SW_Biomass')
    df_pw = forest_cnc_ratio_single('PW_Biomass')
    df_wc = forest_cnc_ratio_single("WoodChips")
    df_sd = forest_cnc_ratio_single("Sawdust")
    df = pd.concat([df_sw, df_pw, df_wc, df_sd])
...
def globiom_forest_harvest_residue_potential():
    ...
    df["YIELD_Total_harvest_residue_m3/ha"] = (df["YIELD_stemwood_m3/ha"] / 0.6 * 0.85 -
                                               df[
                                                   "YIELD_sawlogs+pulpwood_m3/ha_underbark"] / 0.864) * 0.864  # underbark
    ...
    df_nc["RPRODUCT"] = "Logging residue, non-conifer"
    df_nc['TYPE'] = 'NC'
...
def pivot_globiom_crop_data_c():
    df_crop = import_globiom_crop_data()
    table_crop = pd.pivot_table(df_crop, values=["VALUE"], index=['CROP', 'COUNTRY', 'SCENARIO', 'YEAR'],
                                columns=["ITEM"], aggfunc='sum')
    table_crop = table_crop.droplevel(level=0, axis=1)
    table_crop.reset_index(inplace=True)
    table_crop['YIELD'] = table_crop['production'] / table_crop['harvest_area']
...
def all_residue_available_potential_g_no_scenario():
    ...
    df = pd.pivot_table(df0, index=['CAT1', 'CAT2', 'RESIDUE', 'UNIT', 'LU_GRID', 'COUNTRY', 'YEAR'],
                        columns='SCENARIO', values=['AVAI_MIN', 'AVAI_MAX'], aggfunc='sum')
    df = df.fillna(0)
    df.reset_index(inplace=True)
    ...
    df = df[[('CAT1', ''),  ('CAT2', ''), ('RESIDUE', ''), ('UNIT', ''), ('LU_GRID', ''),
             ('COUNTRY', ''), ('YEAR', ''), ('AVAI_MAX', ''), ('AVAI_MIN', '')]].copy()
    df = df.droplevel(1, axis=1)
    df.reset_index(inplace=True)
    return df


def crop_and_wood_product_index():
    dfc0 = pivot_globiom_crop_data_g()
    dff0 = compile_globiom_forest_r_data()[0]
    dfc0 = dfc0[dfc0.YEAR < 2051].copy()
    dff0 = dff0[dff0.YEAR < 2051].copy()
    dfc = pd.pivot_table(dfc0, index=['CROP', 'SCENARIO'], columns='YEAR', values='production', aggfunc='sum')
    dff = pd.pivot_table(dff0, index=['SCENARIO'], columns='YEAR', values='PRODUCTION_M', aggfunc='sum')
    return dfcotential_g()
    df_forest_harvest = forest_harvest_residue_potential_g()
    df_forest_process = forest_process_residues_g()
    df = pd.concat([df_crop, df_forest_harvest, df_forest_process], ignore_index=True)
    df.to_csv('data/interim/GLOBIOM_all_residue_g_processed.csv')
    return df


def export_all_residues_c():
    df_g = export_all_residues_g()
    df_country = get_country_match_df_globiom()
    df_g['Country'] = df_g['COUNTRY'].map(df_country.set_index('GLOBIOM')['ISO2'])
    df_g['Product'] = df_g['RESIDUE']
    df_c = df_g.groupby(by=['CAT1', 'CAT2', 'Product', 'Country', 'SCENARIO', 'YEAR']).sum(numeric_only=True)
    df_c.reset_index(inplace=True)
